...
class Args:
    def __init__(self) -> None:
        ...
        self.max_step = 40 # CG最大迭代次数
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 检测GPU
        self.seed = 10 # 随机种子，置0则不设置随机种子
        self.process_num = mp.cpu_count()  # 每次训练的进程数
        self.train_eps = 200 # 训练的回合数
        self.test_eps = 10 # 测试的回合数
        ################################################################################
        
        ################################## 算法超参数 ####################################
        self.batch_size = 128  # 每次训练的batch大小
...
if __name__ == "__main__":
    ...
    for pi in range(process_num):
        p = mp.Process(target=Trainer.trainOffPolicy, args=(policy, args, res_queue))
        p.start()
        processes.append(p) 
    res = []
    while True:
