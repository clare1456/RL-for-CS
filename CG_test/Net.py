...
class GAT4(nn.Module):
    def __init__(self, node_feature_dim, column_feature_dim, embed_dim, device, hidden_dim=256, heads=8, dropout=0.0):
        ...
        self.conv1 = GATConv(hidden_dim, hidden_dim, heads=heads, dropout=dropout).to(device) 
        self.conv2 = GATConv(hidden_dim*heads, hidden_dim, dropout=dropout).to(device) 
    ...
    def forward(self, state):
        ...
        embeddings = F.relu(self.conv1(embeddings, edges)) 
        embeddings = F.relu(self.conv2(embeddings, torch.flip(edges, [0]))) 
        logits = self.output(embeddings[-len(column_features):]) 
        return logits
